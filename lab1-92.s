...
;;; Getkeywithcounter subroutine ;;;
Getkeywithcounter:
	move.l d0,-(a7)		; throw on stack
	move.l #1000000,d0	; init loop var
waitkeycounter:
	sub.l #1,d0			; decrement loop var
	beq endgetkeycounter

	btst #4,$10082		; check strobe
	beq waitkeycounter	; loop if not pressed

relkeycounter:
	btst #4,$10082		; check strobe
	bne relkeycounter	; loop if still pressed
	move.b $10082,d4	; move PIAB to d4
	and.b #15,d4		; isolate key data
	rts

endgetkeycounter:
	move.b #$A,d4		; fake key to deactivate alarm
